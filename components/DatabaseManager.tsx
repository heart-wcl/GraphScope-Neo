...
import {
  Database, Plus, Trash2, Loader2, RefreshCw,
  Play, Square, CheckCircle, Server,
...
const DatabaseManager: React.FC<DatabaseManagerProps> = ({
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'offline': return <Square className="w-4 h-4 text-gray-400" />;
  ...
  if (loading) {
    return <LoadingOverlay text="加载数据库信息..." />;
  }
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title="数据库管理"
      description={serverInfo ? `${serverInfo.name} ${serverInfo.version} (${serverInfo.edition})` : undefined}
      icon={<Database className="w-5 h-5 text-blue-400" />}
      iconColorClass="bg-blue-500/10"
    >
      {/* Header Actions */}
      <div className="p-4 border-b border-neo-border flex items-center justify-end gap-2">
        <button
          onClick={loadData}
          ...
          {databases.map(db => (
            <div
              ...
              className={`bg-neo-bg rounded-xl border transition-colors ${
                currentDatabase === db.name ? 'border-blue-400' : 'border-neo-border hover:border-blue-400/50'
              }`}
            >
              <div className="p-4">
                <div className="flex items-start justify-between">
                  <div className="flex items-start gap-3">
                    <div className="mt-1">{getStatusIcon(db.currentStatus)}</div>
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-white">{db.name}</span>
                        {db.default && (
                          <span className="text-xs bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded">默认</span>
                        )}
                        {db.home && (
                          <span className="text-xs bg-green-500/20 text-green-400 px-2 py-0.5 rounded">主数据库</span>
                        )}
                        {currentDatabase === db.name && (
                          <span className="text-xs bg-neo-primary/20 text-neo-primary px-2 py-0.5 rounded">当前</span>
                        )}
                      </div>
                      <div className="flex items-center gap-4 mt-1 text-sm text-neo-dim">
                        <span className={getStatusColor(db.currentStatus)}>{db.currentStatus}</span>
                        <span>{db.type}</span>
                        <span>{db.access}</span>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {db.currentStatus === 'offline' && db.name !== 'system' && (
                      <button
                        ...
                        title="启动"
                      >
                        {actionLoading === db.name ? <Loader2 className="w-4 h-4 animate-spin" /> : <Play className="w-4 h-4" />}
                      </button>
                    )}
                    {db.currentStatus === 'online' && db.name !== 'system' && db.name !== 'neo4j' && (
                      <button
                        ...
                        title="停止"
                      >
                        {actionLoading === db.name ? <Loader2 className="w-4 h-4 animate-spin" /> : <Square className="w-4 h-4" />}
                      </button>
                    )}
                    {db.name !== 'system' && db.name !== 'neo4j' && (
                      <button
                        ...
                        title="删除"
                      >
                        {actionLoading === db.name ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h-4" />}
                      </button>
                    )}
                  </div>
                </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="font-medium text-white">{formatNumber(statistics[db.name].nodeCount)}</div>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="font-medium text-white">{formatNumber(statistics[db.name].relationshipCount)}</div>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="font-medium text-white">{statistics[db.name].labelCount}</div>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <GitBranch className="w-4 h-4 text-neo-dim" />
                        <div>
                          ...
                          <div className="font-medium text-white">{statistics[db.name].relationshipTypeCount}</div>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="font-medium text-white">{statistics[db.name].propertyKeyCount}</div>
                        </div>
                      </div>
                    </div>
                  </div>
